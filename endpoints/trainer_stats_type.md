# `/race-cards/trainer-stats-type`

## Endpoint

```http id="o4k8m2"
GET /race-cards/trainer-stats-type
```

## Description

Returns trainer statistics based on the race type of the current race. Each item represents one horse/runner and includes the trainer’s historical performance in races of the same type.

For example, if the current race type is `Flat`, the `runs`, `wins`, `Win %`, `place`, `Place %`, and `total_winnings` fields describe how the trainer has performed historically in Flat races.
...
| `race_type`        | `"Flat"`           | Race type used for the trainer statistics.                           |
| `topSpeed`         | `null`             | Top speed value, if available.                                       |
| `price`            | `null`             | Betting price or odds, if available.                                 |
| `runs`             | `"188"`            | Number of trainer runs in this race type.                            |
| `wins`             | `"32"`             | Number of trainer wins in this race type.                            |
| `Win %`            | `"17%"`            | Trainer win percentage in this race type.                            |
| `place`            | `"69"`             | Number of trainer places in this race type.                          |
| `Place %`          | `"37%"`            | Trainer place percentage in this race type.                          |
| `total_winnings`   | `"313254"`         | Total trainer winnings in this race type.                            |
