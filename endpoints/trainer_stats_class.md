# `/race-cards/trainer-stats-class`

## Endpoint

```http
GET /race-cards/trainer-stats-class
```

## Description

Returns trainer statistics based on the race class of the current race. Each item represents one horse/runner and includes the trainer’s historical performance in races of the same class.

For example, if the current race is a Class 6 race, the `runs`, `wins`, `Win %`, `place`, `Place %`, and `total_winnings` fields describe how the trainer has performed historically in Class 6 races.
...
| `race_class`       | `"6"`              | Race class used for the trainer statistics.                          |
| `topSpeed`         | `null`             | Top speed value, if available.                                       |
| `price`            | `null`             | Betting price or odds, if available.                                 |
| `runs`             | `"61"`             | Number of trainer runs in this race class.                           |
| `wins`             | `"15"`             | Number of trainer wins in this race class.                           |
| `Win %`            | `"25%"`            | Trainer win percentage in this race class.                           |
| `place`            | `"27"`             | Number of trainer places in this race class.                         |
| `Place %`          | `"44%"`            | Trainer place percentage in this race class.                         |
| `total_winnings`   | `"76460"`          | Total trainer winnings in this race class.                           |
